...
def get_tasks_continual(train, test, train_model_input, test_model_input, lf_columns, df_columns):
    
    tmi1 = {
        column : train_model_input[column].iloc[0:len(train_model_input[column]//3)] for column in train_model_input.keys()
    }
    tmi2 = {
        column : train_model_input[column].iloc[len(train_model_input[column]//3):int(len(train_model_input[column]) * (2/3))] for column in train_model_input.keys()
    }
    tmi3 = {
        column : train_model_input[column].iloc[int(len(train_model_input[column]) * (2/3)):] for column in train_model_input.keys()
    }
    
    testmi1 = {
        column : test_model_input[column].iloc[0:len(test_model_input[column]//3)] for column in test_model_input.keys()
    }
    testmi2 = {
        column : test_model_input[column].iloc[len(test_model_input[column]//3):int(len(test_model_input[column]) * (2/3))] for column in test_model_input.keys()
    }
    testmi3 = {
        column : test_model_input[column].iloc[int(len(test_model_input[column]) * (2/3)):] for column in test_model_input.keys()
    }
    
    return [
        (train.iloc[0:len(train)//3], test.iloc[0:len(test)//3], tmi1, testmi1, lf_columns,df_columns),

        (train.iloc[len(train)//3:int(len(train)*(2/3))], test.iloc[len(test)//3:int(len(test)*(2/3))], tmi2, testmi2,lf_columns, df_columns),

        (train.iloc[int(len(train)*(2/3)):], test.iloc[int(len(test) * (2/3)):], tmi3, testmi3, lf_columns, df_columns)
    ]

def get_data(args, start=0, stop=1000000):
    name = args.task_name
...
#             print(type(train_model_input['user_id']))
            train, test, train_model_input, test_model_input, lf_columns, df_columns = ctrdataset(path, start, stop)
...
#             if (continual):
#             return get_tasks_continual(train, test, train_model_input, test_model_input, lf_columns, df_columns)
...
if __name__ == "__main__":
    ...
    if args.task_name == 'ctr':
        if args.model_name == 'din' or args.model_name == 'dien':
            ...
            train, test, train_model_input, test_model_input, lf_columns, df_columns = get_data(args, start=0, stop=10000000 * 12)
#             task1, task2, task3 = get_data(args)
#             tasks = [task1, task2, task3]
#             train, test, train_model_input, test_model_input, lf_columns, df_columns = task1
        ...
        for i in range(12):
            print("task:", i)
            if (i == 0):
                train, test, train_model_input, test_model_input, lf_columns, df_columns = get_data(args, start=0, stop=10000000)
                pass
            else:
                new_train, new_test, new_train_model_input, new_test_model_input, new_lf_columns, new_df_columns = get_data(args, start = 10000000*i, stop=10000000*(i+1))
                train = pd.concat([train, new_train])
                test = pd.concat([test, new_test])
                train_model_input = {
                    col : pd.concat([train_model_input[col], new_train_model_input[col]]) for col in train_model_input.keys()
                }
                test_model_input = {
                    col : pd.concat([test_model_input[col], new_test_model_input[col]]) for col in test_model_input.keys()
                }
            history, best_model = model.fit(train_model_input, train['click'].values, batch_size=args.train_batch_size, epochs=args.epochs, verbose=2, validation_split=0.1111)
        
            pred_ans = best_model.predict(test_model_input, args.test_batch_size)
            print("test LogLoss on task", i, ":", round(log_loss(test['click'].values, pred_ans), 4))
            print("test AUC on task", i, ":", round(roc_auc_score(test['click'].values, pred_ans), 4))
